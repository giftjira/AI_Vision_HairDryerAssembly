...
class MQTTIngest:
    ...
    def _on_message(self, client, userdata, msg):  # pragma: no cover - network
        ...
        if start_time <= current_time <= end_time and "cycle_time" in topic:
            # filtering rule preserved:
            if ct > (std_time or 0) * 0.6:
                try:
                    csv_in_q.put_nowait({"ts": now.isoformat(), "pid": process_id, "ct": float(f"{ct:.1f}")})
                except Exception:
                    pass
...
class DY08Dashboard:
    ...
    def refresh(self, db: Optional[DBConfig] = None) -> None:
        if self.pause:
            return
        now = dt.datetime.now()
        load_csv_incremental(self.state, self.data_dir, self)

        if db is not None and (now - self.last_db_refresh).total_seconds() >= REFRESH_DB_SEC:
            self._refresh_db()
            self.last_db_refresh = now

        # Build compact numeric payload for the stats worker
        # base_time is selection or latest ts
        all_ts = [s.ts for p in self.state.processes.values() for s in p.samples]
        base_dt = self._sel_time or (max(all_ts) if all_ts else dt.datetime.now())
        base_ts = base_dt.timestamp()

        data_payload = {}
        for pid, series in self.state.processes.items():
            if pid == "TT":  # optional include/exclude
                continue
            if not series.samples:
                data_payload[pid] = np.empty((0,2), dtype=np.float64)
                continue
            arr = np.array([[s.ts.timestamp(), s.ct] for s in series.samples if s.ct < 600], dtype=np.float64)
            data_payload[pid] = arr

        try:
            if stats_in_q.empty():  # avoid piling up when UI is fast
                stats_in_q.put_nowait({"base_ts": base_ts, "data": data_payload})
        except Exception:
            pass

        # drain any completed stats result
        try:
            while True:
                latest_stats = stats_out_q.get_nowait()
                self._latest_stats = latest_stats  # stash on dashboard
        except Exception:
            pass

        # drain any DB snapshot
        try:
            while True:
                latest_db = db_out_q.get_nowait()
                # apply without DB calls on UI thread
                self.prod_data = latest_db.get("prod", None)
                self.leader    = latest_db.get("leader", None)
                self.actual    = latest_db.get("actual", None)
                self.tt        = latest_db.get("takt", None)
                std_tuple      = latest_db.get("std_tuple", (None, None, None))
                if std_tuple and std_tuple[1] is not None:
                    mp_adj = std_tuple[1]
                    self.state.std_time = mp_adj
                    self.state.ul_time  = mp_adj * UL_FACTOR
        except Exception:
            pass
    ...
    def _update_live_camera(self, _):
        ...
        if self.live_im is None:
            self.ax_live.axis("off")
            self.live_im = self.ax_live.imshow(frame_rgb, animated=True, aspect='auto')
        else:
...
def _compute_stats_for_pid(args):
    pid, values_ts_ct, base_ts = args
    # values_ts_ct: Nx2 array [unix_ts, ct]
    import numpy as np
    if values_ts_ct.size == 0:
        return (pid, dict(avg=0.0, mn=0.0, mx=0.0, fl=0.0, last5=[]))
    ts = values_ts_ct[:,0]
    ct = values_ts_ct[:,1]
    mask = ts <= base_ts
    ct = ct[mask]
    ...
    mn = float(last5.min())
    mx = float(last5.max())
    fl = float(((mx - mn) / (av if av else 1.0)) * 100.0)
    # keep newest-first for your stats table
    return (pid, dict(avg=av, mn=mn, mx=mx, fl=fl, last5=list(last5[::-1])))

def stats_worker(in_q: Queue, out_q: Queue, stop_ev: Event):
    # fan out work across all CPUs for each batch
    with ProcessPoolExecutor(max_workers=max(1, cpu_count()-1)) as pool:
        while not stop_ev.is_set():
            try:
                payload = in_q.get(timeout=0.2)  # dict: {'base_ts': float, 'data': {pid: Nx2 array}}
            except Exception:
                continue
            if payload == "__STOP__":
                break
            base_ts = payload['base_ts']
            data = payload['data']  # dict pid -> np.ndarray Nx2 (float64)
            jobs = [(pid, arr, base_ts) for pid, arr in data.items()]
            results = dict(pool.map(_compute_stats_for_pid, jobs))
            out_q.put(results)

def db_worker(line: str, date_: dt.date, interval_s: int, out_q: Queue, stop_ev: Event):
    last = 0.0
    while not stop_ev.is_set():
        now = time.time()
        if now - last >= interval_s:
            last = now
            try:
                prod_rows_model = fetch_model_id(line, date_)
                leader = fetch_leader(line, date_)
                actual = fetch_actual(line, date_)
                takt   = fetch_takt_time(line, date_)
                std_tuple = (None, None, None)
                if prod_rows_model:
                    model = prod_rows_model[1]
                    std_tuple = fetch_std_time(model, line, date_)
                out_q.put({
                    "prod": prod_rows_model,
                    "leader": leader,
                    "actual": actual,
                    "takt": takt,
                    "std_tuple": std_tuple
                ...
                # don't spam; you can log to a file if needed
                pass
        time.sleep(0.05)

def csv_writer_worker(path: str, in_q: Queue, stop_ev: Event, flush_every=50, flush_secs=2.0):
    ...
    while not stop_ev.is_set():
        wrote = False
        try:
            item = in_q.get(timeout=0.2)  # {'ts': iso, 'pid': str, 'ct': float}
            if item == "__STOP__": break
            buf.append(item)
        except Exception:
            pass
        if buf and (len(buf) >= flush_every or time.time() - last_flush >= flush_secs):
            for it in buf:
                w.writerow({"timestamp": it["ts"], "process_id": it["pid"], "cycle_time": it["ct"]})
            f.flush()
            os.fsync(f.fileno())
            buf.clear()
            last_flush = time.time()
...
def run_app(args: argparse.Namespace) -> None:
    ...
    # Queues & stop events
    stats_in_q  = Queue(maxsize=2)
    stats_out_q = Queue(maxsize=2)
    db_out_q    = Queue(maxsize=2)
    csv_in_q    = Queue(maxsize=1000)
    stop_ev = Event()

    # Start workers
    P_stats = Process(target=stats_worker, args=(stats_in_q, stats_out_q, stop_ev), daemon=True)
    P_db    = Process(target=db_worker,    args=(lines, today, 60, db_out_q, stop_ev), daemon=True)
    P_csv   = Process(target=csv_writer_worker, args=(csv_filename, csv_in_q, stop_ev), daemon=True)
    P_stats.start(); P_db.start(); P_csv.start()

    # Hold latest snapshots from workers
    latest_stats = {}   # pid -> dict(avg, mn, mx, fl, last5)
    latest_db    = {}   # dict from db_worker
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description=f'{LINE_DEFAULT} Production Dashboard Prototype')
